...
public class AttendanceSpecification {
    ...
    public static Specification<AttendanceEntity> recordedBetween(LocalDateTime start, LocalDateTime end) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (start == null) return criteriaBuilder.lessThanOrEqualTo(root.get("recordedTime"), end);
            if (end == null) return criteriaBuilder.greaterThanOrEqualTo(root.get("recordedTime"), start);
