...
public interface UserMapper {
    ...
    @Mapping(target = "totalLoginTime", constant = "0")
    ...
    // Conditional mapping for sensitive fields
    @Condition
    default boolean isNotSensitiveField(String fieldName) {
        // Define which fields should not be mapped in certain contexts
        List<String> sensitiveFields = List.of("password", "twoFactorSecret", "backupCodes");
        return !sensitiveFields.contains(fieldName);
    }

    // Custom mapping for different contexts
    @Named("publicProfile")
    @Mapping(target = "email", expression = "java(maskEmail(user.getEmail()))")
    @Mapping(target = "phoneNumber", expression = "java(maskPhoneNumber(user.getPhoneNumber()))")
    ...
    // Mapping for analytics and reports
    @Named("analyticsView")
    @Mapping(target = "fullName", expression = "java(user.getFullName())")
    @Mapping(target = "email", ignore = true) // Don't include email in analytics
    @Mapping(target = "phoneNumber", ignore = true)
    @Mapping(target = "address", ignore = true)
    @Mapping(target = "bio", ignore = true)
    UserSummaryResponse toAnalyticsResponse(User user);

    // Mapping for export functionality
    @Named("exportView")
    @Mapping(target = "password", ignore = true)
    ...
    @Mapping(target = "phoneVerificationToken", ignore = true)
    @Mapping(target = "paymentMethodId", ignore = true)
