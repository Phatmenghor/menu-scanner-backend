...
--
-- IMPORTANT: Stop the Spring Boot app before running this script!
--            The ALTER TABLE needs an exclusive lock on the roles table.
--            With the app running (300K users doing EAGER fetch on roles),
--            the lock will wait forever.
-- =====================================================================

-- =====================================================================
-- 0. DROP OLD CONSTRAINTS left over from Hibernate
--    a) roles_name_check — CHECK constraint from when 'name' was an enum
--    b) uk_ofx66keruapi6vyqpv6f2or37 — old UNIQUE on 'name' only
--       (blocks same role name across different businesses)
--    The correct constraint is uk_role_name_business on (name, business_id).
-- =====================================================================
DO $$
DECLARE
    r RECORD;
BEGIN
    -- Drop the old CHECK constraint on name (from enum era)
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE table_name = 'roles'
        AND constraint_name = 'roles_name_check'
        AND constraint_type = 'CHECK'
    ) THEN
        ALTER TABLE roles DROP CONSTRAINT roles_name_check;
        RAISE NOTICE 'Dropped roles_name_check (CHECK)';
    END IF;

    -- Drop any UNIQUE constraint on 'name' alone (not the composite one)
    -- Finds all unique constraints that cover ONLY the 'name' column
    FOR r IN
        ...
        AND tc.constraint_name != 'uk_role_name_business'
        GROUP BY tc.constraint_name
        HAVING COUNT(*) = 1
        AND MAX(ccu.column_name) = 'name'
    LOOP
        EXECUTE format('ALTER TABLE roles DROP CONSTRAINT %I', r.constraint_name);
